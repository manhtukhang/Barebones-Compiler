...
int decode_program(char *fn) {
  ...
  FILE *input;
  char *opcode_list;
  int size = getFileSize(fn);  // get file size
  int i = 0, j = 0;
  ...
  while (i < size) {
    if (j + 1 >= instr_list_size)
      instruct_list = (instruct_t **)realloc(instruct_list,
                                             (j + 10) * sizeof(instruct_t *));

    switch (opcode_list[i]) {
      case LOAD_INS:
      case MOV_INS:  // read 8 byte after byte opcode
        instruct_list[j++] =
            (instruct_t *)decode_ins(opcode_list[i], opcode_list + i + 1, 8);
        i += 9;
        break;
      case JE_INS:
      case CLEAR_INS:
      ...
      case JMP_INS:  // read 4 byte after byte opcode
        instruct_list[j++] =
            (instruct_t *)decode_ins(opcode_list[i], opcode_list + i + 1, 4);
        i += 5;
        break;
    }
  }
  ...
  instruct_list =
      (instruct_t **)realloc(instruct_list, j * sizeof(instruct_t *));
  return j;
}
...
int calculate_reg(int lengh_program) {
  ...
  for (i = 1; i < lengh_program; i++)
    if (instruct_list[i]->opcode > 1 && instruct_list[i]->opcode < 7)
      if (max < instruct_list[i]->param1) max = instruct_list[i]->param1;
...
void print_register(int length) {
  int i;
  printf("Register: ");
  for (i = 0; i < length; i++) printf("%d ", reg_list[i]);
  printf("\n");
}
...
#ifdef __linux__
  sleep(1);
#elif defined _WIN32 || __MINGW32__ || WIN32
  Sleep(1000);
#endif
}
...
void implement_prorgram() {
  ...
  while (step < lengh_program) {
    ...
    if (!check)
      ...
      check = 0;
    if (debug) debug_mode(opcode, last_step, value1, value2, flag);
  }
}
...
int main(int argc, char **argv) {
  ...
  if (argc > 2) {
    ...
    if (strcmp(argv[2], "-h") == 0) usage(argv[0]);
  }
